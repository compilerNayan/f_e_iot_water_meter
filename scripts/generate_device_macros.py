...
def parse_device_config(ini_path):
    ...
    return sorted(devices, key=lambda x: x[1]), valve_control_pin

def generate_macro_flags(devices, valve_control_pin, format='cmake'):
...
def main():
    ...
    devices, valve_control_pin = parse_device_config(ini_path)
    flags = generate_macro_flags(devices, valve_control_pin, format=format_type)
